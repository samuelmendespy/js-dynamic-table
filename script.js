...
// 2 - Função para filtrar por CPF em ordem de idade crescente
function filterPersonsByCPFSortedByAge() {
  // Filtra pessoas com CPF
  const personsWithCPF = personsData.filter((pessoa) => {
...
function filterPersonsOver50() {
  const filteredPersons = personsData.filter((pessoa) => pessoa.Idade > 50);
